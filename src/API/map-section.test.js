...
describe('map-sections', () => {
  ...
  it('should render sections wiht correct data', () => {
    const data = mapSections(pagesFakeData[0].sections);
    expect(data[1].component).toBe('section.section-two-columns');
  });

  it('should test section with invalid data ', () => {
    const withNoTextImageGrid = mapSections([
      {
        __component: 'section.section-grid',
      },
    ]);
    const withNoComponent = mapSections([{}]);
    expect(withNoTextImageGrid).toEqual([
      {
        __component: 'section.section-grid',
      },
    ]);
  ...
  it('should test section.section-grid with no text_grid or image_grid', () => {
    ...
    expect(withNoTextImageGrid.length).toBe(2);
  });

  it('should map two columns', () => {
  ...
  it('should map grid image with data', () => {
    ...
    expect(data.description).toBe('Lorem');
    expect(data.grid[0].srcImg).toBe('a.svg');
    expect(data.grid[0].altText).toBe('relogio');
  });
});
